...
SELECT DA.TIM_DAY
  ,EXTRACT(YEAR FROM DA.TIM_DAY) * 100 + EXTRACT(MONTH FROM DA.TIM_DAY) as MONTH
  ,EXTRACT(YEAR FROM DA.TIM_DAY) * 100 + EXTRACT(WEEK FROM DA.TIM_DAY) as WEEK
  ,DA.SIT_SITE_ID
  ,DA.ITE_ITEM_ID
  ,DA.VERTICAL
  ,DA.DOM_DOMAIN_AGG1
  ,DA.DOM_DOMAIN_AGG2
  ,DA.ITE_ATT_BRAND
  ,SUM(DA.VISITS_1IF) AS VISITS_1IF
  ,SUM(DA.VISITS_3IF) AS VISITS_3IF
  ,SUM(DA.VISITAS_TOTALES) AS VISITAS_TOTALES
FROM `SBOX_PRICING1P.DMP_IF` DA
