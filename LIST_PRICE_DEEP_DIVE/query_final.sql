...
-- LEFT JOIN WHOWNER.LK_DOM_DOMAINS DOM ON t1.DOM_DOMAIN_ID = DOM.DOM_DOMAIN_ID
WHERE 1=1
...
SELECT OFF.*,
  ...
  IT.ITE_ITEM_STATUS,
  IT.ITE_ITEM_SCHEDULED_FLG,
  IT.ITE_ITEM_SUPERMARKET_FLG,
  K.ITE_ATT_VALUE_NAME AS ITE_ATT_BRAND,
   DOM.VERTICAL,
   DOM.DOM_DOMAIN_AGG1,
   DOM.DOM_DOMAIN_AGG2,
FROM OFFERINGS OFF
...
LEFT JOIN WHOWNER.LK_ITE_ITEM_ATTRIBUTE K ON OFF.SIT_SITE_ID = K.SIT_SITE_ID AND OFF.ITE_ITEM_ID = K.ITE_ITEM_ID AND K.ITE_ATT_ATTRIBUTE_ID = 'BRAND' 
LEFT JOIN WHOWNER.LK_ITE_ITEM_DOMAINS DOM ON OFF.SIT_SITE_ID = DOM.SIT_SITE_ID AND OFF.ITE_ITEM_ID = DOM.ITE_ITEM_ID