WITH BPC_DATA_BOX_AB AS (
    ...
    SELECT F.TIM_DAY
        ...
        ,CAST(F.COMP_PRICE_RIVAL AS FLOAT64) AS COMP_PRICE_RIVAL
        --,F.VISITS_TOTAL
    ...
    WHERE FAVORABILITY_TYPE = 'BOX_1P_AB'
...
BPC_DATA_BOX_A AS (
    ...
    SELECT F.TIM_DAY
        ...
        , CAST(COALESCE(F.VISITS_MATCH,0)- COALESCE(F.VISITS_EXPENSIVE,0) AS FLOAT64) AS VISITS_COMPETITIVE
    FROM `WHOWNER.BT_COM_FAVORABILITY`F 
    WHERE FAVORABILITY_TYPE = 'BOX_1P_A'
        AND SIT_SITE_ID IN  (SELECT SIT_SITE_ID FROM SBOX_PRICING1P.LISTA_SITES_1P)
        --AND TIM_DAY >= '2025-02-07' -- ROLLOUT DE COSTO FINANCIERO
        AND DATE_TRUNC(TIM_DAY,MONTH) = DATE_TRUNC(DATE_ADD(CURRENT_DATE(), INTERVAL -1 MONTH),MONTH)
        AND F.VISITS_MATCH > 0
...
ELASTICIDAD AS ( -- Trazer a elasticidade por AGG2
  ...
  SELECT 
    ...
    ITE_ITEM_ID,
    DOM_DOMAIN_AGG2,
    SUM(COALESCE(GMV_IVA_DEDUCTED_LC,0)) AS TGMV_IVA_DEDUCTED_LC,
  FROM WHOWNER.DM_MKP_PL1P_PRICING
  WHERE 1=1
      AND SIT_SITE_ID IN (SELECT SIT_SITE_ID FROM SBOX_PRICING1P.LISTA_SITES_1P)
      AND DATE_TRUNC(TIM_DAY,MONTH) = DATE_TRUNC(DATE_ADD(CURRENT_DATE(), INTERVAL -1 MONTH),MONTH)
      AND GMV_IVA_DEDUCTED_LC > 0

  GROUP BY ALL

  )

  ,INDIVIDUAL_ELASTICITY AS (
SELECT ITEM_ID,
    ELASTICITY_COEFFICIENT,
    LAST_UPDATED,
    MODEL_ID,
    LEFT(ITEM_ID,3) AS SIT_SITE_ID,
    CAST(REGEXP_REPLACE(ITEM_ID, r'\D', '') AS BIGINT) AS ITE_ITEM_ID
  FROM meli-bi-data.WHOWNER.BT_ITEMS_ELASTICITY_FEED
  WHERE LAST_UPDATED < CURRENT_DATE()
    AND DATE_TRUNC(LAST_UPDATED,MONTH) = DATE_ADD(DATE_TRUNC(CURRENT_DATE(), MONTH), INTERVAL -1 MONTH)
  QUALIFY ROW_NUMBER() OVER( PARTITION BY ITEM_ID ORDER BY LAST_UPDATED DESC, CASE WHEN UPPER(MODEL_ID) LIKE ('%BAYESIAN%') THEN 1 WHEN UPPER(MODEL_ID) LIKE ('%FALLBACK%') THEN 2 ELSE 3 END , AUD_INS_DTTM DESC) = 1

  )

  SELECT
    IE.SIT_SITE_ID,
    'NULL' AS DOM_DOMAIN_AGG2,
    IE.ITE_ITEM_ID,
    ELASTICITY_COEFFICIENT AS B,
    FROM INDIVIDUAL_ELASTICITY IE
    WHERE IE.ITE_ITEM_ID IS NOT NULL
...
UNION ALL
  ...
  SELECT  
    ...
    TTI.DOM_DOMAIN_AGG2,
    NULL AS ITE_ITEM_ID,
    SUM(ELASTICITY_COEFFICIENT * TGMV_IVA_DEDUCTED_LC)/SUM(TGMV_IVA_DEDUCTED_LC) AS B,      
  FROM TGMV_TOTAL_ITEM TTI
  LEFT JOIN INDIVIDUAL_ELASTICITY E ON TTI.SIT_SITE_ID = E.SIT_SITE_ID  AND TTI.ITE_ITEM_ID = E.ITE_ITEM_ID
  WHERE TTI.SIT_SITE_ID  IN (SELECT SIT_SITE_ID FROM SBOX_PRICING1P.LISTA_SITES_1P)
    AND ELASTICITY_COEFFICIENT IS NOT NULL
  GROUP BY ALL
...
BPC_PLUS_PRICES AS (
  SELECT COALESCE(B.TIM_DAY, PLM.TIM_DAY) AS TIM_DAY, --FAZER OS COALESCES
        ...
        A.COMP_PRICE_RIVAL AS COMP_PRICE_RIVAL_A,
        COALESCE(A.VISITS_COMPETITIVE,0) AS VISITS_COMPETITIVE_A,
        COALESCE(A.VISITS_MATCH,0) AS VISITS_MATCH_A,
  ...
  FROM BPC_DATA_BOX_AB B
  LEFT JOIN BPC_DATA_BOX_A A ON B.TIM_DAY = A.TIM_DAY AND B.MELI_ID = A.MELI_ID 
...
FINAL_TABLE AS (
    SELECT J.*,
        ...
        CASE WHEN COALESCE(E_ITEM.B,E_AGG2.B,E_TS.B,0) < 0 THEN COALESCE(E_ITEM.B,E_AGG2.B,E_TS.B,0) ELSE 0 END  AS B_EFECTIVO
    FROM JOIN_TABLE J
    LEFT JOIN ELASTICIDAD E_ITEM ON J.SIT_SITE_ID = E_ITEM.SIT_SITE_ID AND J.ITE_ITEM_ID = E_ITEM.ITE_ITEM_ID
    LEFT JOIN ELASTICIDAD E_AGG2 ON J.SIT_SITE_ID = E_AGG2.SIT_SITE_ID AND J.DOM_DOMAIN_AGG2 = E_AGG2.DOM_DOMAIN_AGG2 AND E_AGG2.ITE_ITEM_ID IS NULL
    LEFT JOIN ELASTICIDAD E_TS ON J.SIT_SITE_ID = E_TS.SIT_SITE_ID AND E_TS.DOM_DOMAIN_AGG2 = 'NULL' AND E_TS.ITE_ITEM_ID IS NULL
