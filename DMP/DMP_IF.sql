...
HOUND_1_RAW AS ( 
  ...
  WHERE 
    ...
    AND H.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA') 
...
FROM EXPLOTACION.1P_PRICING_COVERAGE AS C
...
WHERE
  ...
  AND C.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA')
...
SELECT 
  ...
  V.VISITS_TOTALES*PL1P_TIME_IN_STATUS_PERC AS VISITAS_TOTALES,	
FROM EXPLOTACION.1P_PRICING_COVERAGE AS C2
LEFT JOIN `meli-bi-data.SBOX_PRICING1P.VISITAS_TOTALES` AS V 
...
WHERE
  C2.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA') 
