...
EVOLUCION AS ( 
  (SELECT
    ...
    PCB.DOM_DOMAIN_AGG2,
    PCB.ITE_ATT_BRAND,
    UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) AS PL1P_PRICING_CURRENT_WINNING_STRATEGY,
  ...
  --- AGREGO EL CAMPO DE BLACKLIST/WHITELIST
    ...
    CASE 
      ...
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'MANUAL'THEN '2-Manual Strategy'
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NULL THEN '3-Tier wo/CCogs' -- Sin CCogs      
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NULL THEN '4-DoD & Lightning wo/CCogs' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NOT NULL THEN '5-Tier w/CCogs' -- Con CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NOT NULL THEN '6-DoD & Lightning w/CCogs' -- Con CCogs   
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC') 
           AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) 
            IN ('MATCH_EXT',
                'COMPETITORS_PRICE',
...
-- A partir del 8/8/2023 Manual se considera como Automatico por el RO Nodo estandar absorbente  // RO todas las verticales menos CE 1/8/2023, RO Completo 8/8/2023 y se habilita MLA dado que durante semana 33 se encendieron algunos ítems de Apparel
  ...
  --  ) AS PL1P_TIME_IN_STATUS_POND,
    ...
    CASE 
      ...
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NULL THEN '3-Tier wo/CCogs' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NULL THEN '4-DoD & Lightning wo/CCogs' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NOT NULL THEN '5-Tier w/CCogs' -- Con CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NOT NULL THEN '6-DoD & Lightning w/CCogs' -- Con CCogs      
    ...
    CASE 
      ...
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') 
           ...
           IN ('MATCH_EXT',
              ...
              'MANUAL') ---CHANGE
              THEN '7-Automatic Strategies'
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NULL AND DE1.ITE_ITEM_ID IS NULL THEN '3-Tier wo/CCogs' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NULL AND DE1.ITE_ITEM_ID IS NOT NULL THEN '3a-Tier wo/CCogs exception' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NULL AND DE1.ITE_ITEM_ID IS NULL THEN '4-DoD & Lightning wo/CCogs' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NULL AND DE1.ITE_ITEM_ID IS NOT NULL THEN '4a-DoD & Lightning wo/CCogs exception' -- Sin CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') AND A.ITEM_ID IS NOT NULL THEN '5-Tier w/CCogs' -- Con CCogs
      WHEN PCB.SIT_SITE_ID IN ('MLB','MLM','MLC','MLA','MCO') AND UPPER(PCB.PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') AND A.ITEM_ID IS NOT NULL THEN '6-DoD & Lightning w/CCogs' -- Con CCogs       
    ELSE 'REVISAR' END AS TYPE_DETAIL_PRICING
  FROM WHOWNER.DM_PL1P_PRICING_PERFORMANCE PCB
...
SELECT *
  ,CASE
    ...
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'ADJUSTED_PRICE' THEN 'External match'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'ADJUSTED_SCORE_A_PRICE' THEN 'External match'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'BUYBOX_WINNER_PRICE' THEN 'BB Winner'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'COMPETITORS_SCORE_A_PRICE' THEN 'External match'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('DEAL','CAMPAIGN', 'CUSTOM') THEN 'Deals & Promos'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'FALLBACK_PRICE' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'FALLBACK_STD' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'HISTORIC_PRICE' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'LIST_PRICE' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'MANUAL' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'MARKDOWN' THEN 'Markdown'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'MARKDOWN_PRICE' THEN 'Markdown'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'MATCH_EXT' THEN 'External match'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'COMPETITORS_PRICE' THEN 'External match'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'PROFITABILITY_PRICE' THEN 'Profit floor'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) IN ('PROMO','DEAL_OF_THE_DAY','LIGHTNING') THEN 'Deals & Promos'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'TARGET_PRICE' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'BASE_PRICE' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'FALLBACK_MSRP' THEN 'Base pricing'
    WHEN UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) = 'LIST_PRICE_CAP' THEN 'Base pricing'
    ELSE UPPER(PL1P_PRICING_CURRENT_WINNING_STRATEGY) END AS PL1P_PRICING_CURRENT_WINNING_STRATEGY_2
  FROM
  (SELECT 
    E.TIM_DAY,
    CAST(
