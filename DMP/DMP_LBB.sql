...
SELECT
    ...
    CASE WHEN EXTRACT (YEAR FROM DATE_TRUNC(BB.TIM_DAY, WEEK)) = EXTRACT(YEAR FROM DATE_ADD(DATE_TRUNC(BB.TIM_DAY, WEEK), INTERVAL 6 DAY)) 
      THEN
        CONCAT(CAST(DATE_TRUNC(BB.TIM_DAY, WEEK)  AS STRING), " to ",CAST(DATE_ADD(DATE_TRUNC(BB.TIM_DAY, WEEK), INTERVAL 6 DAY) AS STRING))
      ELSE 
        CONCAT(CAST(DATE_TRUNC(BB.TIM_DAY, WEEK)  AS STRING), " to ",CAST(DATE(EXTRACT (YEAR FROM DATE_TRUNC(BB.TIM_DAY, WEEK)),12,31) AS STRING)) 
      END AS WEEK_DETAIL,    BB.SIT_SITE_ID,
    BB.ITE_ITEM_ID,
    BB.VERTICAL,
    CASE 
...
LEFT JOIN WHOWNER.LK_ITE_ITEMS LKI ON BB.SIT_SITE_ID = LKI.SIT_SITE_ID AND BB.ITE_ITEM_ID = LKI.ITE_ITEM_ID
