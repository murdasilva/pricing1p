SELECT
  FAV.TIM_DAY,
    EXTRACT(YEAR FROM FAV.TIM_DAY) *100 + EXTRACT(WEEK FROM FAV.TIM_DAY) AS WEEK,
    ...
    CASE WHEN EXTRACT (YEAR FROM DATE_TRUNC(FAV.TIM_DAY, WEEK)) = EXTRACT(YEAR FROM DATE_ADD(DATE_TRUNC(FAV.TIM_DAY, WEEK), INTERVAL 6 DAY)) 
      ...
      ELSE 
        CONCAT(CAST(DATE_TRUNC(FAV.TIM_DAY, WEEK)  AS STRING), " to ",CAST(DATE(EXTRACT (YEAR FROM DATE_TRUNC(FAV.TIM_DAY, WEEK)),12,31) AS STRING)) 
      END AS WEEK_DETAIL,
  FAV.SIT_SITE_ID,
  FAV.ITE_ITEM_ID,
  FAV.VERTICAL,
